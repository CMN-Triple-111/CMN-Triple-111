...
def post_pic(form_pix):
    random_hax = secrets.token_hex(8)
    _, f_ext = os.path.splitext(form_pix.filename)
    pix_pix = random_hax + f_ext
    pix_post = os.path.join(app.root_path, 'static/profile', pix_pix)
    return pix_post
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title=form.title.data, content=form.content.data, author=current_user, pix=form.pix.data)
        db.session.add(post)
        db.session.commit()
        flash('Your post has been created!', 'success')
        return redirect(url_for('home'))
    return render_template('create_post.html', title='New Post', form=form, legend='New Post', pix=pix)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent with instructions to reset your password. '
              'Check your spam folder if you do not see the reset link.', 'info')
        return redirect(url_for('login'))
